var ResObj = {
    ...
    add:function () {
        $.Hmodal({
            ...
            preprocess:function () {
                ...
                $.getJSON("/auth/resource/node",function (data) {
                    var arr = new Array();
    ...
    updateTheme:function () {
        ...
        $.getJSON("/auth/resource/queryTheme",{
            res_id:id,
            theme_id:theme_id
        },function (e) {
            if (e.length === 0){
    ...
    tree:function (resId) {
        ...
        $.getJSON("/auth/menu/all/except/button",{
            resId:resId
        },function (data) {
            if ( data === null || data.length === 0){
                ...
                $(data).each(function(index,element){
                    ...
                    ijs.parentId = element.Res_up_id;
                ...
                $("#h-auth-resource-list-info").HtreeWithLine({
                    ...
                    onChange:function(obj){
                        ...
                        var theme_id = $("#h-auth-resource-theme-style-code").val();
                        $.getJSON("/auth/resource/queryTheme",{res_id:id,theme_id:theme_id},function (e) {
                            if (e.length===0){
                                $("#h-auth-resource-show-res-bg-color").html("-");
                                $("#h-auth-resource-show-res-class").html("-");
                                $("#h-auth-resource-show-res-img").html("-");
    ...
    configTheme:function(){
        ...
        $.Hmodal({
            ...
            callback:function (hmode) {
                ...
                var res_id = $("#h-auth-resource-show-id").html();
                $.HAjaxRequest({
                    url:"/auth/resource/config/theme",
                    type:"Put",
                    data: {
                        res_id: res_id,
...
$(document).ready(function() {
    ...
    $.getJSON("/auth/resource/subsystem",function (data) {
        var arr = new Array();
        $(data).each(function (index, element) {
            var e = {};
            e.id = element.resId;
